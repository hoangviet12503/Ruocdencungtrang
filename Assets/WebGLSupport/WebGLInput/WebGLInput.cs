...
namespace WebGLSupport
{
    internal class WebGLInputPlugin
    {
        ...
        [DllImport("__Internal")]
        public static extern int WebGLInputCreate(string canvasId, int x, int y, int width, int height, int fontsize, string text, string placeholder, bool isMultiLine, bool isPassword, bool isHidden, bool isMobile, bool isInputNumber);
        ...
        public static int WebGLInputCreate(string canvasId, int x, int y, int width, int height, int fontsize, string text, string placeholder, bool isMultiLine, bool isPassword, bool isHidden, bool isMobile, bool isInputNumber) { return 0; }
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        public void OnSelect()
        {
            ...
            bool isInputNumber = input.contentType == ContentType.IntegerNumber;

            var fontSize = Mathf.Max(14, input.fontSize); // limit font size : 14 !!

            // モバイルの場合、強制表示する
            var isHidden = true;// !(showHtmlElement || Application.isMobilePlatform);
            id = WebGLInputPlugin.WebGLInputCreate(WebGLInput.CanvasId, rect.x, rect.y, rect.width, rect.height, fontSize, input.text, input.placeholder, input.lineType != LineType.SingleLine, isPassword, isHidden, Application.isMobilePlatform, isInputNumber);
